module "kube-hetzner" {
  ...
  # * Your ssh public key
  ssh_public_key = base64decode(var.ssh_public_key_b64) #file("./id_rsa.pub")
  ...
  ssh_private_key = base64decode(var.ssh_private_key_b64) # file("./id_rsa")
  ...
  #ssh_hcloud_key_label = "sshkey=roman"
...
#    kube-scheduler-arg = "bind-address=0.0.0.0",
  ...
  #   },
     ...
     {
       description = "To Allow ArgoCD access to registry"
       direction       = "out"
       protocol        = "tcp"
       port            = "4567"
       source_ips      = [] # Won't be used for this rule
     ...
     {
       description = "To Allow ArgoCD access to registry2"
       direction       = "out"
       protocol        = "tcp"
       port            = "5005"
       source_ips      = [] # Won't be used for this rule
...
variable "ssh_public_key_b64" {
  description = "Base64 закодированный публичный SSH ключ"
  type        = string
}

variable "ssh_private_key_b64" {
  description = "Base64 закодированный приватный SSH ключ"
  type        = string
}
